(function(){
  ...
  function comparePanel(){
    return `<section class="nsHomePanel nsCompare"><div class="nsCompareHead"><div><h2>Utvikling mot markedet</h2><p>Din beholdningsmiks mot valgt indeks · start = 0 %</p></div><div class="nsCompareControls"><select id="nsCompareSelect" class="nsCompareSelect" aria-label="Velg sammenligningsindeks"><option value="OSEBX">OSEBX</option><option value="OSEAX">OSEAX</option><option value="SP500">S&P 500</option><option value="NASDAQ100">Nasdaq 100</option><option value="OMXS30">OMX Stockholm 30</option><option value="DAX">DAX</option></select><div class="nsPeriods">${[['1m','1M'],['3m','3M'],['1y','1Å'],['3y','3Å'],['5y','5Å']].map(([id,label])=>`<button class="nsPeriod${id===comparePeriod?' active':''}" data-period="${id}">${label}</button>`).join('')}</div></div></div><div id="nsCompareBody"><div class="nsCompareLoading">Laster sammenligning…</div></div></section>`;
  }

  function pathFor(rows,key,min,max,w=900,h=260,p=28){
    const span=Math.max(0.0001,max-min),count=Math.max(1,rows.length-1),pts=[];
    rows.forEach((r,i)=>{const v=Number(r[key]);if(!Number.isFinite(v))return;const x=p+(w-p*2)*(i/count),y=p+(h-p*2)*(1-(v-min)/span);pts.push((pts.length?'L':'M')+x.toFixed(1)+' '+y.toFixed(1))});
    return pts.join(' ');
  }

  function chartMarkup(d){
    const rows=(d.series||[]).filter(x=>Number.isFinite(Number(x.portfolio_pct))&&Number.isFinite(Number(x.benchmark_pct)));
    if(rows.length<2)return '<div class="nsEmptyHome"><b>Ikke nok historikk til sammenligning</b>Prøv en kortere periode eller en annen indeks.</div>';
    const values=[];rows.forEach(x=>{values.push(Number(x.portfolio_pct),Number(x.benchmark_pct))});values.push(0);
    let min=Math.min(...values),max=Math.max(...values);const pad=Math.max(1,(max-min)*.12);min-=pad;max+=pad;
    const w=900,h=260,p=28,span=Math.max(.0001,max-min),zeroY=p+(h-p*2)*(1-(0-min)/span);
    const portfolioPath=pathFor(rows,'portfolio_pct',min,max,w,h,p),benchmarkPath=pathFor(rows,'benchmark_pct',min,max,w,h,p);
    const pCls=Number(d.portfolio_return_pct)>=0?'nsPos':'nsNeg',bCls=Number(d.benchmark_return_pct)>=0?'nsPos':'nsNeg',diffCls=Number(d.difference_pp)>=0?'nsPos':'nsNeg';
    const labels=[max,(max+min)/2,min].map(v=>{const y=p+(h-p*2)*(1-(v-min)/span);return `<text x="${p}" y="${Math.max(10,y-4)}" class="nsChartLabel">${Number(v).toLocaleString('no-NO',{maximumFractionDigits:1})}%</text>`}).join('');
    return `<div class="nsCompareStats"><div class="nsCompareStat"><span>Min beholdning · ${esc(d.period_label)}</span><b class="${pCls}">${pct(d.portfolio_return_pct)}</b></div><div class="nsCompareStat"><span>${esc(d.benchmark_label)}</span><b class="${bCls}">${pct(d.benchmark_return_pct)}</b></div><div class="nsCompareStat"><span>Forskjell</span><b class="${diffCls}">${d.difference_pp==null?'—':pct(d.difference_pp).replace('%',' pp')}</b></div></div><div class="nsChartWrap"><svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" role="img" aria-label="Portefølje sammenlignet med ${esc(d.benchmark_label)}"><line x1="${p}" x2="${w-p}" y1="${zeroY}" y2="${zeroY}" class="nsChartZero"></line><line x1="${p}" x2="${w-p}" y1="${p}" y2="${p}" class="nsChartGrid"></line><line x1="${p}" x2="${w-p}" y1="${h-p}" y2="${h-p}" class="nsChartGrid"></line>${labels}<path d="${portfolioPath}" class="nsPortfolioLine"></path><path d="${benchmarkPath}" class="nsBenchmarkLine"></path></svg></div><div class="nsCompareLegend"><span><i class="nsLegendDot portfolio"></i>Min beholdning</span><span><i class="nsLegendDot benchmark"></i>${esc(d.benchmark_label)}</span><span>${d.coverage_pct==null?'':Number(d.coverage_pct).toLocaleString('no-NO',{maximumFractionDigits:0})+'% historikkdekning'}</span></div><div class="nsCompareNote">${esc(d.note||'')}</div>`;
  }

  async function loadComparison(){
    const body=document.getElementById('nsCompareBody'),select=document.getElementById('nsCompareSelect');if(!body)return;
    if(select)select.value=compareBenchmark;
    document.querySelectorAll('.nsPeriod').forEach(b=>b.classList.toggle('active',b.dataset.period===comparePeriod));
    body.innerHTML='<div class="nsCompareLoading">Laster sammenligning…</div>';
    const mine=++compareSeq;
    try{
      const r=await fetch('/api/holdings/compare?benchmark='+encodeURIComponent(compareBenchmark)+'&period='+encodeURIComponent(comparePeriod),{cache:'no-store'}),d=await r.json().catch(()=>({}));
      if(mine!==compareSeq)return;if(!r.ok)throw Error(d.detail||('HTTP '+r.status));body.innerHTML=chartMarkup(d);
