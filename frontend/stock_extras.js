(function(){
  ...
  const compactKr=v=>{if(v==null)return '—';const n=Number(v),a=Math.abs(n);if(a>=1e9)return (n/1e9).toLocaleString('no-NO',{maximumFractionDigits:2})+' mrd. kr';if(a>=1e6)return (n/1e6).toLocaleString('no-NO',{maximumFractionDigits:2})+' mill. kr';if(a>=1e3)return (n/1e3).toLocaleString('no-NO',{maximumFractionDigits:1})+'k kr';return kr(n)};
  ...
  function basisBadge(basis){
    if(basis==='reported_transaction_price')return '<span class="valueBadge actual">FAKTISK</span>';
    if(basis==='market_close_estimate'||basis==='mixed')return '<span class="valueBadge estimate">ESTIMAT</span>';
    return '';
  }

  function valueDetail(value,basis){
    if(value==null)return '<div class="valueHint">Verdi ikke tilgjengelig</div>';
    const hint=basis==='reported_transaction_price'?'Oppgitt transaksjonskurs':basis==='mixed'?'Blanding av oppgitt kurs og markedsestimat':'Sluttkurs rundt handelsdagen';
    return `<div class="valueLine">${esc(compactKr(value))}${basisBadge(basis)}</div><div class="valueHint">${esc(hint)}</div>`;
  }
  ...
  function insiderBehaviorPanel(d){
    ...
    const from=d.actor_history_observed_from||'—',to=d.actor_history_observed_to||'—',livePrice=Number(window.data&&window.data.quote&&window.data.quote.price);
    const rows=h.map(x=>{
      const net=x.net_observed_shares,netCls=net>0?'positive':net<0?'negative':'',currentValue=Number.isFinite(livePrice)&&net!=null?net*livePrice:null;
      ...
      const currentValueHtml=currentValue==null?'':`<div class="valueLine">${esc(compactKr(currentValue))}</div><div class="valueHint">beregnet verdi ved siste kurs · ${esc(fmt(livePrice,2))} kr</div>`;
      return `<tr><td><b>${esc(x.actor||'—')}</b>${actorLabel?`<span class="actorBadge ${actorType}">${actorLabel}</span>`:''}<div class="muted">${esc(x.role||'—')}</div></td><td><span class="behaviorBadge ${behaviorClass(x.pattern)}">${esc(x.pattern_label||'Aktivitet')}</span></td><td>${esc(x.buy_count||0)}<div class="muted">${fmt(x.known_buy_shares,0)} aksjer</div>${valueDetail(x.observed_buy_value,x.observed_buy_value_basis)}</td><td>${esc(x.sell_count||0)}<div class="muted">${fmt(x.known_sell_shares,0)} aksjer</div>${x.sell_count?valueDetail(x.observed_sell_value,x.observed_sell_value_basis):''}</td><td><b class="behaviorNet ${netCls}">${net==null?'—':(net>0?'+':'')+fmt(net,0)}</b><div class="muted">kun observert flyt</div>${currentValueHtml}</td><td>${esc(x.last_buy_date||'—')}</td><td>${esc(x.last_sell_date||'—')}</td><td class="behaviorDetail">${behaviorDetail(x)}</td></tr>`;
    }).join('');
    return `<div class="insiderBehavior"><h3>Aktørhistorikk · kjøp → hold → salg</h3><div class="behaviorMeta">Aksjer-ledger observert fra <b>${esc(from)}</b> til <b>${esc(to)}</b>. Verdier merket <b>FAKTISK</b> bruker oppgitt transaksjonskurs. <b>ESTIMAT</b> bruker sluttkurs rundt handelsdagen når meldingen ikke oppgir pris. Dagens markedsverdi bruker siste tilgjengelige kurs. «Ingen senere salg funnet» betyr kun at Aksjer ikke har registrert en senere offentlig insiderhandel for samme aktør.</div><div class="tablewrap"><table><thead><tr><th>Person / foretak</th><th>Mønster</th><th>Kjøp</th><th>Salg</th><th>Netto obs. / verdi nå</th><th>Siste kjøp</th><th>Siste salg</th><th>Oppfølging</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
  }
  ...
  function installInsider(){
    ...
    window.insider=function(){
      ...
      const note='<div class="notice">Kun offentlige og verifiserbare opplysninger vises. Aksjer følger samme person/foretak over tid og kobler senere salg mot tidligere observerte kjøp. Klassifiseringen beskriver observerbar handelsatferd – ikke investorens motiv.</div>';
      ...
      const rows=a.map(x=>{const actor=x.person||x.entity||x.insider||x.holder||'Ikke oppgitt i kilden',actorType=x.actor_type==='company'?'company':x.actor_type==='person'?'person':'',actorLabel=actorType==='company'?'FORETAK':actorType==='person'?'PERSON':'',value=x.display_transaction_value!=null?x.display_transaction_value:x.transaction_value!=null?x.transaction_value:(x.shares!=null&&x.price!=null?x.shares*x.price:null),basis=x.transaction_value_basis||(x.price!=null?'reported_transaction_price':null),displayPrice=x.price!=null?kr(x.price):x.reference_close_price!=null?'~'+kr(x.reference_close_price):'—',ownSource=x.ownership_pct_source||'',ownBadge=ownSource==='disclosed'?'<span class="sourceBadge disclosed">OPPGITT</span>':ownSource?'<span class="sourceBadge estimated">ESTIMERT</span>':'',ownNote=ownSource==='estimated_from_latest_annual_share_count'?'<small>Basert på siste tilgjengelige aksjetall</small>':'';return `<tr><td>${esc(x.trade_date||x.date||'—')}</td><td><span class="pill ${x.transaction_type==='sell'?'sell':''}">${esc(x.transaction_type==='buy'?'KJØP':x.transaction_type==='sell'?'SALG':'ANNET')}</span></td><td><b>${esc(actor)}</b>${actorLabel?`<span class="actorBadge ${actorType}">${actorLabel}</span>`:''}${x.actor_pattern_label?`<div class="muted">${esc(x.actor_pattern_label)}</div>`:''}</td><td>${esc(x.role||'—')}</td><td>${fmt(x.shares,0)}</td><td>${esc(displayPrice)}${x.reference_close_price!=null&&x.price==null?'<div class="valueHint">sluttkurs-estimat</div>':''}</td><td>${value==null?'—':esc(compactKr(value))}${basisBadge(basis)}</td><td>${fmt(x.holding_after_shares,0)}</td><td class="ownershipCell"><b>${percent(x.ownership_pct)}</b>${ownBadge}${ownNote}</td><td>${x.url?`<a href="${esc(x.url)}" target="_blank" rel="noopener">Original</a>`:'—'}</td></tr>`}).join('');
