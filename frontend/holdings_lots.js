(function(){
  ...
  function initializeForm(x){return `<div class="lotInit"><div class="fieldMini"><label>Kjøpsdato for eksisterende ${num(x.shares)} aksjer · valgfritt</label><input id="lotInitDate${Number(x.id)}" type="date" max="${todayISO()}"></div><button class="btn" onclick="initializePurchaseLots(${Number(x.id)})">Registrer eksisterende kjøp</button></div>`}
  function addForm(x){return `<div class="lotForm"><div><label>Kjøpsdato · valgfritt</label><input id="lotDate${Number(x.id)}" type="date" max="${todayISO()}"></div><div><label>Antall</label><input id="lotShares${Number(x.id)}" type="number" min="0.000001" step="any" placeholder="f.eks. 20"></div><div><label>Pris per aksje · NOK</label><input id="lotPrice${Number(x.id)}" type="number" min="0.000001" step="any" placeholder="f.eks. 22"></div><div class="lotNote"><label>Notat · valgfritt</label><input id="lotNote${Number(x.id)}" maxlength="240" placeholder="f.eks. kjøp etter Q2"></div><button class="btn primary" onclick="savePurchaseLot(${Number(x.id)})">Legg til kjøp</button></div>`}
  ...
  window.initializePurchaseLots=async function(id){const input=$('lotInitDate'+Number(id)),purchase_date=input?.value||null;try{await request(`/holdings/${Number(id)}/purchases/initialize`,{method:'POST',body:JSON.stringify({purchase_date})});expanded.add(Number(id));await loadHoldings()}catch(e){alert(e.message)}};
  window.savePurchaseLot=async function(id){id=Number(id);const purchase_date=$('lotDate'+id)?.value||null,shares=Number($('lotShares'+id)?.value),price_nok=Number($('lotPrice'+id)?.value),note=$('lotNote'+id)?.value?.trim()||null;if(!Number.isFinite(shares)||shares<=0||!Number.isFinite(price_nok)||price_nok<=0){alert('Fyll inn antall og kjøpspris.');return}try{await request(`/holdings/${id}/purchases`,{method:'POST',body:JSON.stringify({purchase_date,shares,price_nok,note})});expanded.add(id);activeAddId=null;await loadHoldings()}catch(e){alert(e.message)}};
