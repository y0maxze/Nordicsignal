(function(){
  ...
  function cleanNavigation(){
    const labels={Dashboard:'Min oversikt','Short Radar':'Short',Markets:'Markeder'};
    document.querySelectorAll('#nav a[data-page]').forEach(a=>{const key=a.dataset.page;if(labels[key])a.textContent=labels[key]});
    const insider=document.querySelector('#nav a[data-page="Insider Activity"]');
    if(insider){insider.textContent='Insider';insider.removeAttribute('data-page');insider.setAttribute('href','/insider')}
    const byHref={'/stock':'Stock Intelligence','/paper':'Paper Trading','/news':'Nyheter','/calendar':'Kalender','/insider':'Insider'};
  ...
  // Home dashboard is the critical path. Load it first. Insider now has a dedicated
  // /insider route, so its large UI module is not downloaded on the home page.
