(function(){
  ...
  function mountNav(){
    ...
    const style=document.createElement('style');style.id='nsMobileShellStyle';style.textContent=`
      ...
      @media(max-width:900px){body{padding-bottom:78px!important}.main{padding-bottom:96px!important}.section,.card{max-width:100%}.table{display:block;overflow-x:auto;-webkit-overflow-scrolling:touch;white-space:nowrap}.nsGlobalHome{display:none!important}.nsMobileNav{display:grid;position:fixed;z-index:2147483000;left:8px;right:8px;bottom:max(8px,env(safe-area-inset-bottom));grid-template-columns:repeat(6,1fr);background:rgba(13,13,13,.97);border:1px solid #2b2b2b;border-radius:15px;box-shadow:0 18px 50px rgba(0,0,0,.6);backdrop-filter:blur(15px);overflow:hidden}.nsMobileNav a{color:#9c9c9c!important;text-decoration:none!important;padding:10px 1px 9px;text-align:center;font:700 8px Inter,system-ui,-apple-system,"Segoe UI",sans-serif}.nsMobileNav a.active{background:#181818;color:#fff!important}.nsMobileNavIcon{display:block;font-size:15px;line-height:18px;margin-bottom:3px;color:#eaeaea}.nsMobileModal{position:fixed;display:flex;z-index:2147483646;inset:0;background:rgba(0,0,0,.82);align-items:center;justify-content:center;padding:16px}.nsMobileModal[hidden]{display:none}.nsMobileModalCard{width:min(460px,100%);background:#0d0d0d;border:1px solid #303030;border-radius:14px;padding:18px;color:#f4f4f4}.nsMobileModalHead{display:flex;justify-content:space-between;align-items:center}.nsMobileModalHead strong{font-size:18px}.nsMobileModalHead button{font-size:24px;border:0;background:transparent;color:#ddd}.nsMobileModalCard p,.nsMobileModalCard li{color:#c4c4c4;line-height:1.6}.nsMobileMuted{color:#8f8f8f;font-size:11px}}
    `;document.head.appendChild(style);
    const p=location.pathname;const active=name=>name==='home'?(p==='/mobile'||p==='/mobile/') : name==='insider'?p.startsWith('/insider') : name==='investment'?(p.startsWith('/readiness')||p.startsWith('/investment-check')) : name==='signals'?(p.startsWith('/stock')||p.startsWith('/intelligence')||p.startsWith('/instrument')) : name==='learning'?(p.startsWith('/learning')||p.startsWith('/signal-performance')) : name==='news'?p.startsWith('/news'):false;
    const nav=document.createElement('nav');nav.id='nsMobileNav';nav.className='nsMobileNav';nav.setAttribute('aria-label','NordicSignal mobilnavigasjon');nav.innerHTML=`<a href="/mobile" class="${active('home')?'active':''}"><span class="nsMobileNavIcon">⌂</span>Oversikt</a><a href="/insider" class="${active('insider')?'active':''}"><span class="nsMobileNavIcon">◎</span>Insider</a><a href="/readiness" class="${active('investment')?'active':''}"><span class="nsMobileNavIcon">✓</span>Investment</a><a href="/stock" class="${active('signals')?'active':''}"><span class="nsMobileNavIcon">↗</span>Signaler</a><a href="/learning" class="${active('learning')?'active':''}"><span class="nsMobileNavIcon">◌</span>Læring</a><a href="/news" class="${active('news')?'active':''}"><span class="nsMobileNavIcon">◫</span>Nyheter</a>`;document.body.appendChild(nav);
  }
