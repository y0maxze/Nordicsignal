(function(){
  ...
  let state='NEW',type='ALL',all=[],loadVersion=0;

  const esc=value=>String(value??'').replace(/[&<>"]/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[ch]));
  const sourceURL=value=>{try{const u=new URL(value);return u.protocol==='https:'?u.href:null}catch{return null}};
  const fmtDate=value=>{if(!value)return '—';try{return new Intl.DateTimeFormat('nb-NO',{timeZone:'Europe/Oslo',dateStyle:'medium',timeStyle:String(value).length>10?'short':undefined}).format(new Date(value))}catch{return String(value)}};
  const fmtNum=value=>{if(value==null||value==='')return null;const n=Number(value);return Number.isFinite(n)?new Intl.NumberFormat('nb-NO',{maximumFractionDigits:2}).format(n):null};
  const fmtMoney=value=>{if(value==null||value==='')return null;const n=Number(value);if(!Number.isFinite(n))return null;if(Math.abs(n)>=1e9)return `${(n/1e9).toFixed(2)} mrd. kr`;if(Math.abs(n)>=1e6)return `${(n/1e6).toFixed(2)} mill. kr`;return `${new Intl.NumberFormat('nb-NO',{maximumFractionDigits:0}).format(n)} kr`};
  ...
  function render(){
    ...
    list.innerHTML=rows.map(x=>{
      ...
      const source=sourceURL(x.source_url)?`<a class="cfSourceLink" href="${esc(sourceURL(x.source_url))}" target="_blank" rel="noopener noreferrer">${esc(x.source||'Kilde')}</a>`:esc(x.source||'Kilde');
      return `<article class="cfPanel cfItem"><div><div class="cfTop"><span class="cfBadge ${esc(String(x.state||'').toLowerCase())}">${esc(stateLabel(x.state))}</span><span class="cfBadge ${esc(x.evidence_level)}">${evidence}</span><span class="cfBadge quality ${esc(String(x.quality_band||"CONTEXT").toLowerCase())}">${qualityLabel(x.quality_band)}${Number.isFinite(Number(x.research_quality))?` · ${esc(x.research_quality)}`:""}</span><span class="cfBadge">${esc(typeLabel(x.event_type))}</span>${x.ticker?`<a class="cfTicker" href="/stock?ticker=${encodeURIComponent(x.ticker)}">${esc(x.ticker)}</a>`:''}</div><div class="cfTitle">${esc(x.title)}</div><div class="cfSummary">${esc(detail||x.summary||'Kapital-/eierendring registrert.')}</div><div class="cfMeta"><span>${esc(fmtDate(x.event_at))}</span><span>${source}</span>${x.instrument_name?`<span>${esc(x.instrument_name)}</span>`:''}</div></div><div class="cfDirection ${esc(direction)}">${dtext}</div></article>`
    }).join('');
  }
  ...
  async function loadOwnership(){
    ...
    try{
      ...
      stateEl.textContent=(status.snapshots||0)>0?'POINT-IN-TIME AKTIV':'UTILGJENGELIG · ingen eierfeed';
      if(incoming){
        ...
        stateEl.textContent=data.status==='ok'?'DATERT SNAPSHOT':'UTILGJENGELIG · ingen snapshot';
  ...
  async function load(force=false){
    ...
    try{const qs=new URLSearchParams({limit:'250',state});const res=await fetch(`/api/capital-flow?${qs}`,{cache:'no-store'});if(!res.ok)throw new Error(`HTTP ${res.status}`);const data=await res.json();if(version!==loadVersion)return;all=Array.isArray(data.items)?data.items:[];const counts=data.counts||{};document.getElementById('cfNew').textContent=counts.NEW??0;document.getElementById('cfActive').textContent=counts.ACTIVE??0;document.getElementById('cfHistorical').textContent=counts.HISTORICAL??0;live.textContent=`Oppdatert ${fmtDate(data.generated_at)} · ${all.length} hendelser lastet`;render()}catch(err){if(version!==loadVersion)return;all=[];list.innerHTML='<div class="cfPanel cfEmpty">UTILGJENGELIG · kunne ikke hente kapitalflyt.</div>';live.textContent='Kunne ikke hente kapitalflyt. Prøv igjen.'}finally{if(version===loadVersion)refresh.disabled=false}
  }
