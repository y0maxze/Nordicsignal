...
## 2026-09-25 — source-reviewed listing profiles

`backend/data/ipo_reviewed_profiles.json` holds reviewed document snapshots, initially Pelican Aqua (PLCAN, CY0201461213, 2026-07-02 admission). Each section has source, publication/as-of date and document locator. `ipo_reviewed_profiles.py` requires exact ticker, ISIN, market and admission date; rows without an exact identity remain unknown. The projection applies only at/after review time, performs no network requests and does not modify any model, coverage flag or historical return series. This is manually maintained research, not an automatic daily company feed or PIT evidence.

Pelican sources were accessed via its issuer website, which links the information document and presentation at storage.mfn.se. The information document's figures are historical USD consolidated figures for 26 March–10 June 2026. Cash at the balance date predates the first yard instalment: the UI explicitly warns against interpreting it as current liquidity. Lock-up duration is shown with person and exceptions, without inventing expiry. Calendar dates are no longer presented as upcoming after they pass. Current valuation and post-period finances remain unknown. No benchmark returns were added.

To add another issuer, review its official documents, confirm all four identity fields, add bounded factual sections with sources/as-of dates and review timestamp, and run identity, no-lookahead, rendering and mandatory release tests. Update snapshots through PR review when newer disclosures change facts; never silently overwrite source dates or imply continuous monitoring.

The legacy `ipo_prelisting_profile_runtime.py` is deliberately not reused for this display: its `_nok` names do not establish source currency, and its EV/EBITDA approximation excludes cash. It needs a separate currency/enterprise-value audit before production presentation. Automated arbitrary PDF extraction is not treated as verified fact.
