...
MARKETS = (("Brent", "BZ=F", "Energi"),("WTI", "CL=F", "Energi"),("S&P 500", "^GSPC", "USA"),("Nasdaq", "^IXIC", "USA"),("Euro Stoxx 50", "^STOXX50E", "Europa"),("USD/NOK", "NOK=X", "Valuta"))
...
def _market_snapshot(provider):
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        ...
        for future in as_completed(futures):
            ...
            except Exception as exc:errors.append(f"{label}: {exc}"); rows.append({"label":label,"symbol":symbol,"group":group,"change_pct":None,"status":"unavailable"})
    order={item[0]:i for i,item in enumerate(MARKETS)}; rows.sort(key=lambda x:order.get(x.get("label"),99)); return rows,errors


def _risk(event):
    days=int(event.get("days_until") or 0); kind=event.get("event_type")
    if kind=="report" and days<=1:return "critical"
    if kind=="report" and days<=3:return "high"
    if kind in ("report","meeting") and days<=7:return "watch"
    if kind=="dividend" and days<=3:return "watch"
    return "normal"


def _event_sort_key(item):
    risk_rank={"critical":0,"high":1,"watch":2,"normal":3}; return (risk_rank.get(item.get("risk"),9),int(item.get("days_until") or 0),0 if item.get("tracked") else 1,item.get("ticker") or item.get("company") or "")


def _radar_risk(priority):return "high" if priority=="high" else "watch" if priority=="watch" else "normal"


def _must_know_sort(item):
    rank={"critical":0,"high":1,"watch":2,"normal":3}; source_rank=0 if item.get("source_context")=="calendar" else 1; materiality=(item.get("materiality") or {}).get("revenue_ratio_pct"); materiality_rank=-float(materiality) if isinstance(materiality,(int,float)) else 0
    return (rank.get(item.get("risk"),9),source_rank,materiality_rank,int(item.get("days_until") or 99))


def build_morning_brief(days=7,news_limit=12,now=None,provider=None,force=False):
    days=max(1,min(int(days or 7),30)); news_limit=max(1,min(int(news_limit or 12),30)); current=now or datetime.now(timezone.utc); cache_key_ok=days==7 and news_limit==12 and now is None
    with _CACHE_LOCK:
        if cache_key_ok and not force and _CACHE["value"] is not None and time.time()-_CACHE["at"]<_CACHE_TTL:return dict(_CACHE["value"])
...
def install():
    if getattr(extra_api,"_morning_brief_runtime_v1",False):return
    original_install=extra_api.install
    def patched_install(app):
        original_install(app)
        @app.get("/api/morning-brief")
        def morning_brief(days:int=7,news_limit:int=12,refresh:bool=False):return build_morning_brief(days=days,news_limit=news_limit,force=refresh)
    extra_api.install=patched_install; extra_api._morning_brief_runtime_v1=True
