    ...
    try:
        for raw in items or []:
            ...
            existing = conn.execute(
                "SELECT candidate_id FROM ipo_candidates WHERE candidate_id=?",
                (candidate_id,),
            ).fetchone()
            payload = json.dumps(raw, ensure_ascii=False, separators=(",", ":"), default=str)
            if existing:
                conn.execute(
                    "UPDATE ipo_candidates SET last_seen_at=?,raw_payload=? WHERE candidate_id=?",
                    (now, payload, candidate_id),
                )
                continue
            conn.execute(
                ...
                (
                    ...
                    raw.get("market"),
                    raw.get("expected_listing_date_text"),
                    raw.get("offer_price_nok"),
                    raw.get("title"),
                    raw.get("source_url"),
...
def recent_candidates(since=None, limit=20):
    ...
    try:
        if since:
            rows = conn.execute(
                "SELECT * FROM ipo_candidates WHERE first_seen_at>? ORDER BY first_seen_at DESC LIMIT ?",
                (str(since), max(1, min(int(limit or 20), 100))),
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT * FROM ipo_candidates ORDER BY first_seen_at DESC LIMIT ?",
...
def archive_status(limit=20):
    ...
    return {
        ...
        "count": len(rows),
        "items": rows,
        "policy": "Verified IPO candidate archive only. No score or threshold changes.",
        "generated_at": _now(),
    }
