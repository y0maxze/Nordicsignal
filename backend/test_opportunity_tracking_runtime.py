...
def test_forward_returns_settle_on_trading_day_horizons(tmp_path, monkeypatch):
    ...
    rows = []
    for i in range(70):
        rows.append({"date": f"2026-01-{min(i + 2, 31):02d}" if i < 30 else f"2026-02-{min(i - 28, 28):02d}", "close": 100 + i})
    ...
    rows = [{"date": f"2026-{1 + (i // 28):02d}-{1 + (i % 28):02d}", "close": 100 + i} for i in range(70)]
