...
# GlobeNewswire's organization search can itself be served from a stale cache.
# This category load-more endpoint is refreshed continuously and currently carries
# Lerøy's issuer releases. We only follow links whose URL/title identifies Lerøy,
# then verify the full release before parsing any trade.
FRESH_ISSUER_FEEDS = {
    "LSG": (
...
def _fresh_items(session, ticker, company_name):
    ...
    for feed in FRESH_ISSUER_FEEDS.get(ticker, ()):
        try:
            html = fetch(session, feed)
        except Exception:
            continue
        for url, label in discover_release_links(html, feed, ticker)[:20]:
...
def merge_insider_result(base, fresh, ticker):
    ...
    if fresh:
        ...
        out["fresh_fallback_count"] = len(fresh)
    return out


def install():
    if getattr(NordicRegulatoryProvider, "_fresh_insider_fallback_v1", False):
        return
    ...
    NordicRegulatoryProvider._fresh_insider_fallback_v1 = True
