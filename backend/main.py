...
UNIVERSE = [
    ("LSG", "Lerøy Seafood", "Seafood"), ("MPCC", "MPC Container Ships", "Shipping"),
    ("ELO", "Elopak", "Packaging"), ("PEXIP", "Pexip", "Technology"), ("XPLRA", "Xplora Technologies", "Technology"),
    ("EQNR", "Equinor", "Energy"), ("DNB", "DNB", "Financials"), ("NHY", "Norsk Hydro", "Materials"),
    ("YAR", "Yara International", "Chemicals"), ("MOWI", "Mowi", "Seafood"), ("SALM", "SalMar", "Seafood"),
    ("GJF", "Gjensidige Forsikring", "Financials"), ("TEL", "Telenor", "Telecom"), ("ORK", "Orkla", "Consumer"),
    ("TOM", "Tomra Systems", "Industrials"), ("KOG", "Kongsberg Gruppen", "Industrials"), ("NAS", "Norwegian Air Shuttle", "Airlines"),
    ("AKRBP", "Aker BP", "Energy"), ("AKSO", "Aker Solutions", "Energy"), ("SUBC", "Subsea 7", "Energy"),
    ("BWLPG", "BW LPG", "Shipping"), ("HAUTO", "Höegh Autoliners", "Shipping"), ("GOGL", "Golden Ocean", "Shipping"),
    ("VAR", "Vår Energi", "Energy"),
]
...
def fundamental_metrics(r):
    ...
    revenue, ebitda, net_income = f.get("revenue"), f.get("ebitda"), f.get("net_income")
    fcf, debt, equity = f.get("free_cashflow"), f.get("debt"), f.get("equity")
    ...
    return {"revenue": revenue, "ebitda": ebitda, "net_income": net_income, "free_cashflow": fcf, "debt": debt, "equity": equity,
            "revenue_growth": growth, "ebitda_margin": (ebitda / revenue) if ebitda is not None and revenue else None,
            "roe": (net_income / equity) if net_income is not None and equity else None,
            "debt_to_equity": (debt / equity * 100) if debt is not None and equity else None,
            "fcf_margin": (fcf / revenue) if fcf is not None and revenue else None}


def fundamentals_score(m):
    points = 20.0
    margin, roe, growth, debt_eq, fcf = m["ebitda_margin"], m["roe"], m["revenue_growth"], m["debt_to_equity"], m["free_cashflow"]
...
def valuation_score(r):
    ...
    pe, pb, ev_ebitda = raw(sd.get("trailingPE"), raw(ks.get("trailingPE"))), raw(ks.get("priceToBook")), raw(ks.get("enterpriseToEbitda"))
    points = 10.0
...
def sentiment_score(history):
    ...
    start, end = closes[-min(60, len(closes))], closes[-1]
...
def seed_db():
    ...
    for ticker, name, sector in UNIVERSE:
        ...
        if not conn.execute("SELECT 1 FROM scores WHERE ticker=? LIMIT 1", (ticker,)).fetchone():
            conn.execute("INSERT INTO scores(ticker,fundamentals,insider,valuation,sentiment,total,created_at,source) VALUES(?,?,?,?,?,?,?,?)", (ticker, 20, 12, 10, 8, 50, now, "seed"))
...
def refresh_one(ticker):
    ...
    try:
        ...
        i = insider_score(insider)
        verified_max, verified_sum = 75 + (25 if i is not None else 0), f + v + s + (i or 0)
        normalized = clamp_score((verified_sum / verified_max) * 100, 0, 100)
        source, now = ("live" if i is not None else "partial_live"), datetime.now(timezone.utc).isoformat()
...
def refresh_all(limit=None):
    tickers = TICKERS[:limit] if limit else TICKERS; results = []
    with ThreadPoolExecutor(max_workers=5) as pool:
        futures = {pool.submit(refresh_one, ticker): ticker for ticker in tickers}
...
def startup():
    init_db(); seed_db(); refresh_all()


@app.get("/api/health")
def health():
    return {"status": "ok", "service": "NordicSignal API", "version": "3.0.0", "providers": ["Yahoo Finance", "Finanstilsynet SSR", "Euronext Oslo Børs"], "score_policy": "coverage-aware live score"}


@app.get("/api/refresh")
def refresh(all: bool = True): return {"status": "ok", "results": refresh_all() if all else refresh_all(limit=8)}
...
def short(ticker: str):
    try:
        conn = connect(); row = conn.execute("SELECT name FROM stocks WHERE ticker=?", (ticker.upper(),)).fetchone(); conn.close(); company_name = row["name"] if row else ""
        return regulatory.short(ticker, company_name)
    except Exception as exc: return {"ticker": ticker.upper(), "items": [], "source": "unavailable", "status": "unavailable", "error": str(exc)}
