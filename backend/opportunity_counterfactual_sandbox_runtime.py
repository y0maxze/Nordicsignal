...
CANDIDATES = (
    {
        "id": "baseline_rev75_vol150",
        "name": "Baseline confluence",
        "description": "Reversal >= 75 and bullish volume >= 1.50x.",
        "reversal_min": 75.0,
        "volume_min": 1.50,
        "insider_positive_required": False,
    },
    {
        "id": "research_rev72_vol150",
        "name": "Research: reversal 72",
        "description": "Reversal >= 72 and bullish volume >= 1.50x.",
        "reversal_min": 72.0,
        "volume_min": 1.50,
        "insider_positive_required": False,
    },
    {
        "id": "research_rev75_vol135",
        "name": "Research: volume 1.35x",
        "description": "Reversal >= 75 and bullish volume >= 1.35x.",
        "reversal_min": 75.0,
        "volume_min": 1.35,
        "insider_positive_required": False,
    },
    {
        "id": "research_rev70_vol135_insider",
        "name": "Research: insider-supported",
        "description": "Reversal >= 70, volume >= 1.35x and POSITIVE/STRONG insider evidence.",
        "reversal_min": 70.0,
        "volume_min": 1.35,
        "insider_positive_required": True,
    },
)
...
def _eligible(snapshot, candidate):
    reversal = _number(snapshot.get("reversal_score"))
    volume = _number(snapshot.get("volume_ratio"))
    if reversal is None or volume is None:
        return False
    if reversal < float(candidate["reversal_min"]) or volume < float(candidate["volume_min"]):
        return False
...
def _load_rows(model_id):
    ...
    try:
        snapshots = [dict(row) for row in conn.execute(
            "SELECT id,ticker,market_date,reversal_score,volume_ratio,insider_label FROM opportunity_shadow_snapshots "
            "WHERE signal_model_id=? ORDER BY market_date,id",
            (model_id,),
        ).fetchall()]
        returns = [dict(row) for row in conn.execute(
            "SELECT r.snapshot_id,r.horizon_days,r.return_pct,r.excess_return_pct FROM opportunity_shadow_returns r "
            "JOIN opportunity_shadow_snapshots s ON s.id=r.snapshot_id "
            "WHERE s.signal_model_id=? AND r.horizon_days IN (5,10,20) AND r.return_pct IS NOT NULL AND r.excess_return_pct IS NOT NULL",
            (model_id,),
        ).fetchall()]
...
def _metric(values):
    ...
    if not values:
        return {"n": 0, "median_pct": None, "mean_pct": None, "positive_rate_pct": None}
    return {
        "n": len(values),
        "median_pct": round(float(median(values)), 4),
        "mean_pct": round(sum(values) / len(values), 4),
        "positive_rate_pct": round(sum(value > 0 for value in values) / len(values) * 100.0, 2),
    }
...
def _segment_report(snapshot_ids, returns_by_snapshot):
    ...
    for horizon in HORIZONS:
        ...
        for snapshot_id in snapshot_ids:
            ...
            if not outcome:
                continue
            raw.append(outcome.get("return_pct"))
            alpha.append(outcome.get("excess_return_pct"))
        raw_metric = _metric(raw)
        alpha_metric = _metric(alpha)
...
def sandbox_report():
    ...
    base = {
        ...
        "quality_gate_ready": bool(gate.get("ready_for_counterfactual_research")),
        "automatic_threshold_changes": False,
        "automatic_candidate_ranking": False,
        "candidate_set_locked_in_source": True,
    ...
    returns_by_snapshot = {
        (int(row["snapshot_id"]), int(row["horizon_days"])): row
        for row in returns
    }
    ...
    for candidate in CANDIDATES:
        ...
        results.append({
            ...
            "development_selected": len(development_ids),
            "holdout_selected": len(holdout_ids),
            "development": _segment_report(development_ids, returns_by_snapshot),
            "holdout": _segment_report(holdout_ids, returns_by_snapshot),
        })

    base.update({
        "status": "OPEN_RESEARCH_ONLY",
        "method": "pre_registered_fixed_candidates_chronological_development_holdout",
        "market_days": len(dates),
        "development_market_days": len(development_dates),
        "holdout_market_days": len(holdout_dates),
...
def install():
    if getattr(extra_api, "_opportunity_counterfactual_sandbox_runtime", False):
        return
    ...
    def patched_install(app):
        ...
        def opportunity_shadow_sandbox_route():
