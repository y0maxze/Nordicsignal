...
class YahooProvider(MarketDataProvider):
    ...
    def quote(self, ticker):
        ...
        data = self._get(
            f"{self.BASE}/v8/finance/chart/{symbol}",
    ...
    def fundamentals(self, ticker):
        ...
        for base in self.BASES:
            try:
                data = self._get(
                    f"{base}/ws/fundamentals-timeseries/v1/finance/timeseries/{symbol}",
                    params,
                )
                result = data.get("timeseries", {}).get("result") or []
                if result:
                    # Yahoo returns one result object per metric. Normalize that
                    # list into a metric-name -> datapoints mapping so callers
                    # can safely use series.get(metric, []).
                    series = {}
                    for item in result:
    ...
    def research(self, ticker):
        ...
        revenue = self._latest(flat.get("annualTotalRevenue"))
        ebitda = self._latest(flat.get("annualEBITDA"))
        ebit = self._latest(flat.get("annualEBIT"))
        net_income = self._latest(flat.get("annualNetIncome"))
        eps = self._latest(flat.get("annualDilutedEPS"))
        ocf = self._latest(flat.get("annualOperatingCashFlow"))
        fcf = self._latest(flat.get("annualFreeCashFlow"))
        debt = self._latest(flat.get("annualTotalDebt"))
        equity = self._latest(flat.get("annualStockholdersEquity"))
        gross = self._latest(flat.get("annualGrossProfit"))
        opinc = self._latest(flat.get("annualOperatingIncome"))
        pretax = self._latest(flat.get("annualPretaxIncome"))
        shares = self._latest(flat.get("annualBasicAverageShares"))
