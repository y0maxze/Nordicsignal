...
def _ordered_horizon_rows(rows, horizon):
    """Return one complete raw+alpha observation per event, oldest first."""
    selected = {}
    synthetic = 0
    for row in rows or []:
        try:
    ...
    def sort_key(item):
        ...
        try:
            ...
            numeric = 10**18
        return (observed, numeric, str(item.get("_stable_key") or ""))
...
def _holdout_direction(values):
    ...
    return {
        ...
        "direction_pass": bool(med > 0.0 and positive_rate >= MIN_HOLDOUT_POSITIVE_RATE_PCT),
    }
...
def _folds(rows, minimum_train=INITIAL_TRAIN_EVENTS, holdout_size=HOLDOUT_EVENTS):
    ...
    while train_end + holdout_size <= len(rows):
        ...
        holdout = rows[train_end:train_end + holdout_size]
        result.append((fold_number, train, holdout))
...
def _fold_report(number, train, holdout):
    ...
    holdout_pass = bool(
        len(holdout) == HOLDOUT_EVENTS
        and holdout_raw["direction_pass"]
        and holdout_alpha["direction_pass"]
    )
    eligible = training_ready and len(holdout) == HOLDOUT_EVENTS
    status = "TRAIN_NOT_READY"
    if eligible:
    ...
    return {
        ...
        "training": {
            "n": len(train),
            "first_event_id": train[0].get("event_id") if train else None,
            "last_event_id": train[-1].get("event_id") if train else None,
            "last_observed_at": train[-1].get("observed_at") if train else None,
            "raw_return": train_raw,
            "market_adjusted_alpha": train_alpha,
        },
        "holdout": {
            "n": len(holdout),
            "first_event_id": holdout[0].get("event_id") if holdout else None,
            "last_event_id": holdout[-1].get("event_id") if holdout else None,
            "first_observed_at": holdout[0].get("observed_at") if holdout else None,
            "last_observed_at": holdout[-1].get("observed_at") if holdout else None,
            "raw_return": holdout_raw,
            "market_adjusted_alpha": holdout_alpha,
            "pass": holdout_pass,
        },
        "leakage_guard": {
            "chronological": True,
            "training_ends_before_holdout_starts": bool(
                train and holdout and
                (str(train[-1].get("observed_at") or ""), int(train[-1].get("event_id") or 0))
                <= (str(holdout[0].get("observed_at") or ""), int(holdout[0].get("event_id") or 0))
            ),
        },
    }
...
def _horizon_report(rows, horizon):
    ...
    eligible = [fold for fold in fold_reports if fold["eligible"]]
    ...
    ready = bool(
        eligible_count >= MIN_ELIGIBLE_FOLDS
        and len(passing) >= required_passes
        and latest_pass
    )
    if ready:
        status = "PASS"
    elif eligible_count >= MIN_ELIGIBLE_FOLDS:
        status = "REVIEW"
    else:
        status = "COLLECTING_DATA"
    next_fold_needs = max(0, INITIAL_TRAIN_EVENTS + HOLDOUT_EVENTS * (MIN_ELIGIBLE_FOLDS + 0) - len(ordered))
    return {
        "status": status,
        "ready": ready,
        ...
        "holdout_pass_rate_pct": round(pass_rate, 2) if pass_rate is not None else None,
        "required_passing_folds": required_passes,
        "latest_eligible_holdout_pass": latest_pass if eligible else None,
...
def walkforward_gate(rows, required_horizons):
    horizons = {
        str(int(horizon)): _horizon_report(rows, int(horizon))
        for horizon in required_horizons
    }
    passing_horizons = [
        int(horizon) for horizon in required_horizons
        if horizons[str(int(horizon))]["ready"]
    ]
    ready = len(passing_horizons) >= MIN_REQUIRED_HORIZONS
    enough_any = any(
        item["eligible_folds"] >= MIN_ELIGIBLE_FOLDS
        for item in horizons.values()
    )
    return {
        "status": "PASS" if ready else ("REVIEW" if enough_any else "COLLECTING_DATA"),
        "ready": ready,
        "method": "chronological_expanding_window_no_shuffle",
        "initial_training_events": INITIAL_TRAIN_EVENTS,
        "holdout_events_per_fold": HOLDOUT_EVENTS,
        "minimum_eligible_folds_per_horizon": MIN_ELIGIBLE_FOLDS,
        "minimum_passing_horizons": MIN_REQUIRED_HORIZONS,
        "minimum_holdout_positive_rate_pct": MIN_HOLDOUT_POSITIVE_RATE_PCT,
        "minimum_fold_pass_rate_pct": round(MIN_FOLD_PASS_RATE * 100.0, 2),
        "latest_holdout_must_pass": True,
        "passing_horizons": passing_horizons,
        "horizons": horizons,
        "meaning": (
            "The active model remains directionally positive across enough chronological out-of-sample blocks."
            if ready else
            "The active model does not yet have enough successful chronological holdout evidence for threshold review."
        ),
