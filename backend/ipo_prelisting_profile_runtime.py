...
This layer reports only fields supported by the archived announcement and optional
market/fundamental data. Unknown inputs stay unknown. It never modifies the normal
NordicSignal stock score or promotes an IPO to an investable signal.
"""
...
PROFILE_VERSION = "ipo_prelisting_profile_v1"
_REQUIRED_FIELDS = (
    "listing_terms",
    "valuation",
    "profitability",
...
def _research_for(candidate, provider=None):
    ...
    try:
        ...
        # A genuine pre-listing company commonly has no quote/fundamental endpoint yet.
...
def _financial_profile(research):
    ...
    net_income = _num(financial.get("netIncomeToCommon"))
    debt = _num(financial.get("totalDebt"))
    fcf = _num(financial.get("freeCashflow"))
    shares = _num(stats.get("sharesOutstanding"))
    market_cap = _num(detail.get("marketCap"))
    return {
        "revenue_nok": revenue,
        "ebitda_nok": ebitda,
        "net_income_nok": net_income,
        "free_cashflow_nok": fcf,
        "total_debt_nok": debt,
        "shares_outstanding": shares,
        "market_cap_nok": market_cap,
        "profitable": (net_income > 0) if net_income is not None else None,
        "ebitda_margin_pct": round(ebitda / revenue * 100.0, 2) if revenue and ebitda is not None else None,
    }


def build_profile(candidate, provider=None, research=None):
    ...
    revenue = financial.get("revenue_nok")
    ebitda = financial.get("ebitda_nok")
    ...
    known = sum(1 for key in _REQUIRED_FIELDS if evidence.get(key))
    coverage_pct = round(known / len(_REQUIRED_FIELDS) * 100.0, 1)

    if coverage_pct < 50:
        assessment = "Utilstrekkelig data for investeringsvurdering"
        status = "insufficient"
    elif coverage_pct < 80:
        assessment = "Delvis profil – krever mer dokumentasjon"
        status = "partial"
    else:
        assessment = "Datagrunnlag klart for separat IPO-vurdering"
    ...
    return {
        ...
        "expected_listing_date_text": candidate.get("expected_listing_date_text"),
        "source_url": candidate.get("source_url"),
        "market_data_status": market_data_status,
        "financials": financial,
        "valuation": valuation,
        "ownership": {"founder_retention_pct": None, "lockup": None, "status": "unknown"},
        "offering": {"new_capital_nok": None, "secondary_sale_nok": None, "use_of_proceeds": None, "status": "unknown"},
        "coverage": {"known": known, "total": len(_REQUIRED_FIELDS), "pct": coverage_pct, "fields": evidence},
        "status": status,
        "assessment": assessment,
        "policy": "Pre-listing context only. No stock-score change, no threshold tuning and no investment recommendation.",
        "generated_at": _now(),
    }
...
def install():
    if getattr(extra_api, "_ipo_prelisting_profile_runtime_v1", False):
        return
    ...
    extra_api._ipo_prelisting_profile_runtime_v1 = True
