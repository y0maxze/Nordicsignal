...
def _ipo_push_event(row):
    ...
    return {"event_key":f"ipo:{candidate_id}","ticker":d.get("ticker"),"title":f"Ny børsnotering · {company}","body":" · ".join(details),"url":"/ipo-radar","created_at":d.get("first_seen_at")}
...
def install():
    ...
    def event_rows_with_ipo(since):
        ...
        try:
            ...
            pass
        deduped = {str(x.get("event_key")): x for x in items if x.get("event_key")}
