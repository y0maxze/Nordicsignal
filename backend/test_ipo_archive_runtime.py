...
def test_candidate_id_is_stable():
    ...
    second = archive._candidate_id(candidate())
    assert first == second
    assert len(first) == 32
...
def test_candidate_id_changes_for_different_source():
    first = archive._candidate_id(candidate())
    second = archive._candidate_id(candidate(source_url="https://live.euronext.com/en/node/456"))
    assert first != second


def test_ipo_push_event_contains_stable_key_and_context():
    row = candidate(candidate_id="abc123", first_seen_at="2026-06-08T09:00:00+00:00")
    event = bridge._ipo_push_event(row)
...
def test_archive_rejects_unverified_candidates(monkeypatch):
    ...
    inserted = archive.record_candidates([candidate(official=False)])
    assert inserted == 0
